...
// unit-economics, provider-economics, capability and outreach-scale slides, then
// 21 -> 20 when outreach-scale was deleted as factually wrong (v1.11.0).
...
test('slide counter, nav dots and PPTX slide count are all 20', () => {
  const counters = deck.match(/\d\d \/ 20/g) || [];
  assert.strictEqual(counters.length, 20, `expected 20 "NN / 20" counters, found ${counters.length}`);
  assert.ok(deck.includes('20 / 20'), 'final counter 20 / 20 missing');

  const navDots = deck.match(/class="nav-dot(?: active)?" data-slide=/g) || [];
  assert.strictEqual(navDots.length, 20, `expected 20 nav dots, found ${navDots.length}`);

  const addSlide = deck.match(/pptx\.addSlide\(\)/g) || [];
  assert.strictEqual(addSlide.length, 20, `expected 20 PPTX slides, found ${addSlide.length}`);
});
