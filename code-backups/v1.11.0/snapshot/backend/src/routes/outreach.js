...
const { parseInboundEmail, inboundAuthResult } = require('../services/outreach/inbound.service');
...
router.post('/inbound', async (req, res) => {
  const auth = inboundAuthResult(
    req,
    process.env.RESEND_WEBHOOK_SECRET,
    process.env.NODE_ENV
  );
  if (!auth.ok) {
    // Log the reason server-side: every rejection looks like a 401 from the
    // outside, so without this a misconfigured webhook is indistinguishable
    // from a forged request and there is nothing to debug against.
    console.warn(`[outreach/inbound] rejected: ${auth.code} — ${auth.reason}`);
    return res.status(401).json({
      error: 'Invalid webhook signature',
      code: auth.code,
      // The reason names a configuration fault, not a secret, so it is safe to
      // return and saves a round trip through the logs during setup.
      reason: auth.reason,
    });
  }
