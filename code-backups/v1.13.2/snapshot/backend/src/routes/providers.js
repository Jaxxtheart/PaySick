...
// Dashboard: Collections summary — the ONLY collections visibility a
// provider ever gets. Aggregate and reputational only: PaySick's Recovery
// Engine owns every case end to end, so there is no patient-level detail
// and no action item here for the provider to work.
router.get('/dashboard/collections-summary', authenticateToken, requireRole('provider'), async (req, res) => {
  try {
    const providerId = req.user.provider_id || await resolveProviderId(req.user.user_id);
    if (!providerId) {
      return res.status(404).json({ error: 'No provider linked to this account' });
    ...
    const result = await query(
      `SELECT
        COUNT(DISTINCT p.payment_id) FILTER (WHERE p.status = 'overdue')                          AS overdue_payments_count,
        COUNT(DISTINCT c.collection_id) FILTER (WHERE c.status IN ('open', 'in_progress'))          AS open_collections_cases,
        COUNT(DISTINCT c.collection_id) FILTER (WHERE c.status = 'resolved'
                                                   AND c.resolution_type = 'paid')                   AS cured_count,
        CASE WHEN COUNT(DISTINCT c.collection_id) > 0
             THEN ROUND(
               COUNT(DISTINCT c.collection_id) FILTER (WHERE c.status = 'resolved'
                                                          AND c.resolution_type = 'paid')::NUMERIC
               / COUNT(DISTINCT c.collection_id) * 100, 1)
             ELSE NULL
    ...
    res.json({
      provider_id: providerId,
      overdue_payments_count: parseInt(row.overdue_payments_count) || 0,
      open_collections_cases: parseInt(row.open_collections_cases) || 0,
      cured_count: parseInt(row.cured_count) || 0,
      cure_rate_pct: row.cure_rate_pct !== null && row.cure_rate_pct !== undefined ? parseFloat(row.cure_rate_pct) : null,
    ...
    console.error('Provider collections-summary error:', error);
