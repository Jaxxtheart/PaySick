...
## 9. Test Architecture (Added v1.1.0)

```
tests/
├── setup.js                     # Environment bootstrap for all tests
├── __mocks__/
│   ├── database.js              # Jest mock — replaces pg Pool with in-memory stub
│   └── email.service.js         # Jest mock — prevents real email sends
├── unit/
│   ├── security.service.test.js # Tests: hashPassword, verifyPassword, encrypt/decrypt, sanitize
│   └── security-utils.test.js   # Tests: XSS sanitize, validate SA ID/email/phone, mask, currency
└── integration/
    ├── health.test.js            # Tests: GET /health, GET /, 404 handler
    ├── users.test.js             # Tests: register, login, demo-login, verify-email, tokens, profile
    ├── applications.test.js      # Tests: POST /, GET /, GET /:id (with validation)
    ├── payments.test.js          # Tests: plans, upcoming, history, pay, transactions
    ├── providers.test.js         # Tests: list, search, detail (public routes)
    └── risk.test.js              # Tests: portfolio-summary, distribution (admin-only)
```

### Test Execution

**Unit tests** (zero external deps, runs anywhere with Node 18+):
```bash
node --test tests/unit/*.test.js
```

**Integration tests** (requires `npm install` for Jest + supertest):
```bash
...
Integration tests use `jest.mock()` to replace the PostgreSQL database module
(`backend/src/config/database`) with an in-memory stub. Each test controls
the mock's return values via `mockQuery.mockResolvedValueOnce(...)`. This
means tests are fully deterministic and require no running database.

Authentication in integration tests is simulated by pre-loading the auth mock
to return a valid session row when the middleware queries for the token hash.
