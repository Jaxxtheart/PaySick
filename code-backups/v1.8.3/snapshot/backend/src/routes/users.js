...
router.post('/forgot-password', forgotPasswordRateLimit, async (req, res) => {
  ...
  const ok = { message: 'If an account with that email exists, a password reset link has been sent.' };
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res.json(ok); // Don't reveal whether the email exists
    }
    ...
    // Atomically invalidate any existing unused token and insert the new one.
    // Wrapping in a transaction prevents the race condition where two concurrent
    // requests both read "no active token", both insert, and one silently becomes
    // unreachable (the partial unique index on (user_id) WHERE used=false catches
    // the second insert and rolls it back).
    ...
    await logSecurityEvent('PASSWORD_RESET_REQUESTED', user.user_id, ipAddress,
      req.get('User-Agent'), { email: user.email });
 ...
 * Revokes all active sessions on success (logs out all devices).
 */
router.post('/reset-password', async (req, res) => {
  ...
  try {
    ...
    const { token_id, user_id, expires_at, email } = result.rows[0];
    ...
    await transaction(async (client) => {
      ...
      await client.query(
        `UPDATE user_sessions
         SET revoked = true, revoked_at = NOW(), revoke_reason = 'password_reset'
         WHERE user_id = $1 AND revoked = false`,
        [user_id]
      );
    });

    await logSecurityEvent('PASSWORD_CHANGED', user_id, ipAddress,
      req.get('User-Agent'), { method: 'reset', email });
