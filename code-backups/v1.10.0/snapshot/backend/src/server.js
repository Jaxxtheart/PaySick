...
const sanparksRoutes = require('./routes/sanparks');
...
// The SANParks media surface serves a licensing catalogue and executes rights
// transfers. A catalogue is the most scrape-attractive thing on the platform and
// a rights transfer is irreversible, so it gets its own bucket rather than the
// general read allowance. Reviewed at v1.10.0.
const sanparksLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 40,
  message: {
    error: 'Too many media licensing requests. Please try again later.',
    code: 'SANPARKS_RATE_LIMITED',
    retryAfter: 900
  },
  standardHeaders: true,
  legacyHeaders: false
...
app.use('/api/sanparks', sanparksLimiter);
...
// SANParks Media Licensing — subscriptions, catalogue, licences, chain of title
app.use('/api/sanparks', sanparksRoutes);
