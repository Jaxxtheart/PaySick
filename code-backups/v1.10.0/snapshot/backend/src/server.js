...
// Tighter bucket for the outreach agent surface. Two of its routes are reachable
// without a session: the Resend inbound webhook (public, signature-gated) and the
// daily cron route (secret-gated, and one call runs a Places + Anthropic
// pipeline). Neither should get the allowance of a read endpoint, and the admin
// approve queue never comes close to this ceiling.
...
app.use('/api/outreach', outreachLimiter);
