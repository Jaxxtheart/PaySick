...
const PaySickAPI = {
  ...
  // ============================================
  // SANPARKS MEDIA LICENSING
  // ============================================
  sanparks: {
    /** Plan catalogue with 12- and 24-month term pricing. */
    async plans() {
      return PaySickAPI.request('/sanparks/plans');
    },

    /** The caller's subscriptions, each with its status at this moment. */
    async subscriptions() {
      const data = await PaySickAPI.request('/sanparks/subscriptions');
      return data.subscriptions || [];
    },

    /** Start a subscription. termMonths must be 12 or 24. */
    async subscribe({ planCode, termMonths, licenseeName }) {
      return PaySickAPI.request('/sanparks/subscriptions', {
        method: 'POST',
        body: JSON.stringify({ planCode, termMonths, licenseeName }),
      });
    },

    /** Renew for a further term. Renewals inside grace stay continuous. */
    async renew(subscriptionId, { termMonths } = {}) {
      return PaySickAPI.request(`/sanparks/subscriptions/${subscriptionId}/renew`, {
        method: 'POST',
        body: JSON.stringify({ termMonths }),
      });
    },

    /** Stop auto-renewal. The paid term runs to its end. */
    async cancel(subscriptionId) {
      return PaySickAPI.request(`/sanparks/subscriptions/${subscriptionId}/cancel`, { method: 'POST' });
    },
    ...
    async assets(filters = {}) {
      ...
      const data = await PaySickAPI.request(`/sanparks/assets${params ? `?${params}` : ''}`);
      return data.assets || [];
    },

    /** Chain of title for an asset, with its verification result. */
    async chain(assetId) {
      return PaySickAPI.request(`/sanparks/assets/${assetId}/chain`);
    },

    /** Price a licence without buying it. Returns the quote and any rights blockers. */
    async quote(configuration) {
      return PaySickAPI.request('/sanparks/quotes', {
        method: 'POST',
    ...
    /**
     * Buy the licence. The sale and the rights transfer settle in one
     * transaction — the response carries the licence, the rights certificate and
     * the chain-of-title entry, or nothing happened at all.
     */
    async licence(configuration) {
      return PaySickAPI.request('/sanparks/licences', {
        method: 'POST',
        body: JSON.stringify(configuration),
      });
    },
    ...
    async licences() {
      const data = await PaySickAPI.request('/sanparks/licences');
