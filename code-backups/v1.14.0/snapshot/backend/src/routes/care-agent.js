...
const { extractCareRequest, detectIntent, PROCEDURE_CATEGORIES } = require('../services/care-agent-nlp.service');
const {
  mergeCareSummary,
  missingFieldsFor,
  nextQuestion,
...
router.post('/sessions/:id/messages', authenticateToken, async (req, res) => {
  try {
    ...
    const merged = mergeCareSummary(session.structured_summary || {}, extracted, message);
