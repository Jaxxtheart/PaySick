...
|---------|------|---------|
| [v1.12.0](./v1.12.0/) | 2026-08-04 | MINOR: Removes the four remaining v1.10.0 business-case slides (pricing, unit economics, provider economics, capability value) at the founder's direction, completing the revert of that addition — deck 20→16, back to its pre-v1.10.0 length; dead `.ledger`/`.assumption`/`.pill` CSS removed with them; the superseded-pricing bans migrate into `investor-deck.test.js` so the old figures still cannot reappear. **The deck now carries no pricing or business-model slide at all** — the original was removed in v1.10.0 for contradicting `fee.service.js` and is deliberately not restored. New standing guard asserts all surviving slides by id, not just by count; 19 test-first assertions (683 pass) |
| [v1.11.0](./v1.11.0/) | 2026-08-04 | MINOR: Deletes the "Outreach at Scale" deck slide — it claimed sending was blocked and no message had ever been sent, when 40+ outreach emails had been sent and nothing in the code gates a send (the approve route calls the email service directly); deck 21→20, OUT-01..06 withdrawn, capability slide restated on its own reference scale. Fixes the inbound reply webhook verifying against a re-serialised body (`JSON.stringify(req.body)` can never match Svix-signed bytes), replacing it with `inboundAuthResult()` returning a distinct code per failure mode; documents the Resend setup and warns that an unwired webhook makes follow-ups chase providers who already replied; 26 test-first assertions (694 pass) |
| [v1.10.1](./v1.10.1/) | 2026-08-04 | PATCH: `/investors` returned a Vercel 404 — the deck lives in `investor-deck.html` so `cleanUrls` served it only at `/investor-deck`, and `vercel.json` rewrote only `/health` and `/api/(.*)`. Adds a `/investors` → `/investor-deck.html` rewrite (not a redirect, so shared links keep their path), ordered above the API rules; bot-protection review confirms `robots.txt` and the catch-all `X-Robots-Tag` already cover the new path; 8 test-first assertions (676 pass) |
| [v1.10.0](./v1.10.0/) | 2026-08-04 | MINOR: Investor-deck business case + pricing rebuilt against the code — deck pricing now matches `fee.service.js` (5% provider service fee, R0 patient, 2% placement fee, 5%/month late fee), blended take corrected 10% → 6.2%; new slides for pricing, per-arrangement unit economics (R1,147 gross / R1,009 contribution), per-provider profitability (13-arrangement year-one breakeven, 1.9mo payback, 15.2x LTV:CAC), quantified capability value, and the outreach plan costed blocked / at caps / uncapped (46 vs 273 providers a year); deck 17→21 slides with the 12/13 counter gap fixed; superseded Business Model slide removed; 8 second-wave AI crawlers added at the MINOR blocklist review; 59 new test-first assertions (668 pass) |
