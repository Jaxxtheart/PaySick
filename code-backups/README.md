...
|---------|------|---------|
| [v1.10.0](./v1.10.0/) | 2026-08-08 | MINOR: SANParks Media Licensing — subscription-based licensing of imagery and footage captured on SANParks property, with the image rights transferred and recorded in the same database transaction as the sale; renewable 12/24-month terms with grace, continuity and price-lock rules; pricing by rarity, capped demand surge, scope, territory and duration; conservation levy / contributor royalty / platform fee split reconciling to the cent; exclusivity and buyout conflict detection under a `FOR UPDATE` row lock; hash-linked chain of title with tamper detection and rights certificates; property/model release and sensitive-species geo-redaction gates; `/api/sanparks` (10 endpoints, all authenticated) + `sanparks.html` + migration 010 (7 tables); 16 media harvesters added to the blocklist, dedicated `/api/sanparks` rate bucket; 236 new test-first assertions (845 pass) |
