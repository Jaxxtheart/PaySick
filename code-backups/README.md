...
|---------|------|---------|
| [v1.10.0](./v1.10.0/) | 2026-08-08 | MINOR: Outreach plan expanded to dentists — dental added as an active vertical at launch fit weight, sourced on six Places terms with the daily cap shared evenly across verticals; every message rebuilt around one call to action (register as a provider on the PaySick site) with the 15 minute demo demoted to secondary and hello@paysick.co.za given for anything needed first; no em dash may appear in any message, subject line included; both rules enforced in code at draft, follow-up, onboarding reply, founder edit and approve time; new `style.js` and `cta.js` gates; MINOR security review adds 10 blocklist entries and a dedicated `/api/outreach` rate bucket; 46 new test-first assertions (655 pass) |
