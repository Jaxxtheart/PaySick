      ...
      <table cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
        ...
        <tr><td style="background:#ffffff;padding:40px;border:1px solid #EEEEEE;border-top:none;border-radius:0 0 12px 12px;">
          ...
          <p style="margin:0;color:#AAAAAA;font-size:12px;line-height:1.6;text-align:center;">
            Tech and Artery (Pty) Ltd &mdash; hello@paysick.co.za<br>
...
async function sendMail({ to, subject, html }) {
  ...
  if (process.env.NODE_ENV === 'production' && !process.env.SMTP_HOST) {
    throw new Error('SMTP_HOST is required in production. Configure SMTP environment variables in the Vercel dashboard.');
  }
...
async function sendPasswordResetEmail(to, fullName, rawToken) {
  ...
  const body = `
    <h2 style="margin:0 0 16px;color:#1A1A1A;font-size:22px;font-weight:700;">Reset your password</h2>
    <p style="margin:0 0 12px;color:#4A4A4A;font-size:15px;line-height:1.6;">Hi ${firstName},</p>
    <p style="margin:0 0 24px;color:#4A4A4A;font-size:15px;line-height:1.6;">
      We received a request to reset your PaySick account password.
    ...
    <p style="margin:0 0 24px;word-break:break-all;">
      <a href="${resetUrl}" style="color:#FF4757;font-size:13px;">${resetUrl}</a>
    </p>
    <p style="margin:0;color:#AAAAAA;font-size:12px;line-height:1.6;">
  ...
  return sendMail({
    to,
    subject: 'Reset your PaySick password',
    html: emailWrapper(body),
  });
}

/**
 * Send any pre-rendered journey email (from MessagingJourneyService templates).
 * Called by NotificationService for email-channel dispatches.
 *
 * @param {{ to: string, subject: string, html: string }} opts
 */
async function sendJourneyEmail({ to, subject, html }) {
  if (!to) throw new Error('sendJourneyEmail: recipient `to` is required.');
  return sendMail({ to, subject, html });
}

module.exports = { sendVerificationEmail, sendPasswordResetEmail, sendJourneyEmail };
