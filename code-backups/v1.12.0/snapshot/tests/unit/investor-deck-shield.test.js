...
describe('Shield slide is present and navigable', () => {
  ...
  test('deck counters, nav dots and PPTX slides are all 16', () => {
  ...
  test('no stale "/ 17", "/ 20" or "/ 21" counters left behind', () => {
    // 16 is the live denominator again, so it is deliberately not in this list.
    for (const n of [17, 20, 21]) {
      assert.ok(
