...
describe('POST /api/care-agent/sessions/:id/messages', () => {
  ...
  // This whole file requires jest/supertest, unavailable in this dev
  // sandbox (same constraint noted throughout this repo's RELEASE_NOTES.md
  // since v1.9.0) -- written test-first alongside
  // tests/unit/care-agent-llm.test.js (which IS executable and covers the
  // orchestration loop itself in full). These two document the route's
  // contract: when ANTHROPIC_API_KEY is set, the LLM path is attempted
  // (one extra query -- loading conversation history -- before the same
  // UPDATE + audit INSERT every path ends with); when it fails, the
  // patient still gets a usable reply via the deterministic fallback.
  describe('LLM tool-calling path (ANTHROPIC_API_KEY set)', () => {
    ...
    test('uses the model\'s reply when the Anthropic API call succeeds', async () => {
      global.fetch = jest.fn().mockResolvedValue({
        ok: true,
        json: async () => ({
          stop_reason: 'end_turn',
          content: [{ type: 'text', text: 'Hi there, what brings you in today?' }]
        })
      });

      setupAuthMock([
        { rows: [careAgentSessionRow()], rowCount: 1 }, // getOwnedSession
        { rows: [], rowCount: 0 },                      // loadConversationHistory
        { rows: [], rowCount: 0 },                      // UPDATE session
        { rows: [], rowCount: 0 }                       // INSERT audit log
      ]);
      ...
      expect(res.status).toBe(200);
      expect(res.body.reply).toBe('Hi there, what brings you in today?');
    });

    test('falls back to the deterministic engine when the Anthropic API call fails, and still replies', async () => {
